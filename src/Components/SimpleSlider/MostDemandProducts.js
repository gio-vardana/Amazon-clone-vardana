...
export const MostDemandProducts = () => {
  ...
  useEffect(() => {
    baseAPI.get('api/product/mostdemandproducts')
      .then(response => {
  ...
  return (
    <div className="bg-white border-[1px] m-[24px] mt-[0px] ">
      <h1 className="lg:text-[24px] text-[14px] font-semibold mb-[20px] ml-[10px]">MostDemandProducts</h1>

      <Slider {...settings}>
        {items.map(item => (
          <div key={item.id} className="flex justify-between ml-5">
            <Link to={`/product/${item.id}`}>
              <img
                src={item.images[0]}
                alt={item.description}
                className="lg:h-[255px] h-[230px] bg-cover mb-[10px] ml-[38px]  cursor-pointer"
              />
            </Link>
            ...
            <p className="font-bold pb-[10px] lg:text-[15px] text-[12px]">${item.price}</p>
          </div>
        ))}
      </Slider>
    </div>
